...
export function TimetablePanel({ stopId, routeId, stopName, onClose }: Props) {
  ...
  const modal = (
    ...
    >
      <div
        className="w-full max-w-md max-h-[85vh] rounded-t-3xl sm:rounded-3xl border-2 border-line bg-surface shadow-pop overflow-hidden flex flex-col animate-slide-up"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-start justify-between gap-3 px-5 pt-5 pb-4 border-b-2 border-line shrink-0">
          <div className="flex items-center gap-3 min-w-0 flex-1">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-secondary-500 text-white shrink-0">
        ...
        {/* Day picker */}
        <div className="flex items-center gap-1.5 px-4 py-3 border-b-2 border-line shrink-0">
          {DAYS.map(({ key, label }) => (
            <button
              key={key}
              onClick={() => setDay(key)}
              className={cn(
                ...
                day === key
                  ...
                  : 'bg-primary-50 text-ink2 hover:text-ink'
              )}
            >
              {label}
            </button>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12">
              <div className="flex h-16 w-16 items-center justify-center rounded-3xl bg-coral-50 text-rose-400 mb-3">
            ...
            <div className="space-y-4">
              {[...grouped.entries()].map(([hour, items]) => {
                ...
                return (
                  <div key={hour}>
                    {/* Hour header */}
                    <div className="sticky top-0 bg-surface py-1.5 flex items-center gap-2 border-b-2 border-line mb-2">
                      <span className="text-lg font-extrabold text-primary-700 tabular-nums">
                        {hourDisplay.split(':')[0]}h
                      </span>
                      {nextDay && (
                        <Pill variant="secondary" size="sm">+1</Pill>
                      )}
                    </div>
                    {/* Times */}
                    <div className="space-y-1.5">
                      {items.map((entry, i) => {
                        ...
                        return (
                          <div
                            key={`${entry.tripId}-${i}`}
                            className="flex items-center gap-3 rounded-2xl border-2 border-line bg-bg px-3 py-2"
                          >
        ...
        {/* Disclaimer */}
        <div className="px-5 py-3 border-t-2 border-line bg-sun-400/20 shrink-0">
          <div className="flex items-center gap-2 text-xs text-ink2 font-bold">
            <AlertTriangle className="h-4 w-4 shrink-0 text-sun-500" strokeWidth={2.5} />
